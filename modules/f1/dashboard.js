...
const dashboardModalData = new Map();

function createDashboardCard(title, value, subtitle, icon, modalType, modalData = null) {
    const modalId = `${modalType}-${Math.random().toString(36).slice(2)}`;

    if (modalData !== null) {
        dashboardModalData.set(modalId, modalData);
    }
    ...
    document
        ...
        .forEach(card => {
            ...
            card.addEventListener("click", () => {
                ...
                const data =
                    dashboardModalData.get(modalId) ?? null;
